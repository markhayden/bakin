...
2. **Task log entries from `TASKBOARD.md`** — agents post these via `POST /api/tasks/log`. But agents currently barely use this. Looking at the dispatch message in `dispatch.ts`, agents are *told* to log:
...
This endpoint already exists (handled in `server.ts:190-196`) and broadcasts `{ type: 'activity' }` events. The task log API (`/api/tasks/log`) also fire-and-forgets to `/api/activity/emit`. So the plumbing is there.
...
The current `/api/activity/emit` is perfect for this but it's **not in any agent's dispatch instructions**. Agents only know about `/api/tasks/log`.

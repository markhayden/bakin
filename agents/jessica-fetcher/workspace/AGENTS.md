...
- **You only respond to the agent that invoked you.** Check the task for an `assignedBy` or `author` field. Report back to them — not Roscoe (unless Roscoe created the task directly).

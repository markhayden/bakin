...
const pluginsAndMiscRoutes = [
  ...
  defineCoreRoute({ path: '/api/plugins/upgrade', method: 'POST', summary: 'Upgrade plugin', description: 'Two-phase: preview returns awaitingConsent + consentToken when permissions or binaries widen; commit with accepted + consentToken.', body: z.object({ pluginId: z.string(), accepted: z.boolean().optional(), consentToken: z.string().optional() }), responses: { 200: passthrough, 400: errorResponse, 500: errorResponse }, handler: stub }),

...
function dispatchTasks() {
  ...
  const agents = ['main', 'patch', 'pixel', 'rolo', 'chef']
  let changed = false
  ...
  for (const { line, title, assignedAgent, description } of taskBlocks) {
    ...
    const failureInstructions = `If you cannot complete this task or hit an error, log it: ${logCmd.replace('AGENT_NAME', assignedAgent || 'main-operator').replace('YOUR_STATUS', 'BLOCKED: <reason>')} — then report via: openclaw agent --agent main --message "TASK BLOCKED: ${title} — <reason>" --deliver`

    if (!assignedAgent) {
      targetAgent = 'main'
      message = `Triage this task: "${title}".${detailsBlock}\n\nEither handle it yourself or assign it to the right agent (patch=execution, pixel=design/media, rolo=content/comms, chef=research/strategy) by updating ${taskboardRef}. ${contactsRef}\n\nLog your progress: ${logCmd.replace('AGENT_NAME', 'main-operator').replace('YOUR_STATUS', 'your update here')}`
    } else if (assignedAgent === 'main-operator') {
      targetAgent = 'main'
      message = `Work on this task: "${title}".${detailsBlock}\n\n${contactsRef} When done, move it to the Done column in ${taskboardRef} and log what you did.\n\nLog progress as you work: ${logCmd.replace('AGENT_NAME', 'main-operator').replace('YOUR_STATUS', 'your update here')}\n\n${failureInstructions}`
    } else {
      targetAgent = resolveId(assignedAgent)
      message = `Work on this task: "${title}".${detailsBlock}\n\nLog your progress as you work: ${logCmd.replace('AGENT_NAME', assignedAgent).replace('YOUR_STATUS', 'your update here')}\n\nWhen you are finished, log completion and report back to main-operator using: openclaw agent --agent main --message "TASK COMPLETE: ${title} — <summary of what you did>" --deliver\n\n${failureInstructions}`
    }
    ...
    const agentTag = assignedAgent ? `@${assignedAgent}` : '@main-operator'
  ...
  watcher.on('add', (fullPath: string) => {
    ...
    try {
      ...
      if (msg.type === 'task-complete' && msg.title && msg.agent) {
        // Forward to Main Operator for review

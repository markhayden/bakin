...
export const CLI_COMMANDS = [
  ...
  cli({ name: 'plugins install', usage: 'bakin plugins install [--dev] <path|github:user/repo[#subpath]> [--yes] [--force]', group: 'Plugins', summary: 'Install a plugin.', description: 'Installs a plugin from a local path or GitHub source. Append #subpath to install from a monorepo directory. --dev symlinks a local source tree for live development. --yes skips the consent prompt. --force replaces an existing install when used with --dev.', examples: [{ title: 'Install local plugin for development', code: 'bakin plugins install --dev ./my-plugin', test: 'illustrative', reason: 'Mutates local plugin state.' }] }),

...
export const CLI_COMMANDS = [
  ...
  cli({ name: 'plugins list', usage: 'bakin plugins list [--check] [--json]', group: 'Plugins', summary: 'List installed plugins.', description: 'Lists installed plugins and versions. Pass --check to probe remote/source for available upgrades, or --json for raw API output.', examples: [{ title: 'List plugins', code: 'bakin plugins list', test: 'illustrative', reason: 'Requires local server/plugin state.' }] }),
  ...
  cli({ name: 'plugins upgrade', usage: 'bakin plugins upgrade <id> [--yes] [--json]', group: 'Plugins', summary: 'Upgrade a user plugin.', description: 'Re-pulls a user plugin from its source and rebuilds. --yes skips the consent prompt. Pass --json for raw API output.', examples: [{ title: 'Upgrade plugin', code: 'bakin plugins upgrade my-plugin', test: 'illustrative', reason: 'Mutates local plugin state.' }] }),

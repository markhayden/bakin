...
export const CLI_COMMANDS = [
  ...
  cli({ name: 'models', usage: 'bakin models restore <snapshot.json> [--json]', group: 'Setup and config', summary: 'Restore model selections from a Reset snapshot.', description: 'Diffs a Models-page Reset snapshot (plugin-settings/models/snapshots/*.json) against the CURRENT selections and applies the difference through the validated selections write path under the current revision — undoes a Reset even after later edits.', examples: [{ title: 'Undo the last Reset', code: 'bakin models restore ~/.bakin/plugin-settings/models/snapshots/<file>.json', test: 'illustrative', reason: 'Mutates model configuration.' }] }),

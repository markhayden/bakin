...
export const CLI_COMMANDS = [
  ...
  cli({ name: 'skills install', usage: 'bakin skills install <url-or-ref> [--yes]', group: 'Agents and packages', summary: 'Install a skill from a hub.', description: 'Installs an Agent-Skills bundle from ClawHub, GitHub, or a local path onto the active runtime. Accepts pasted clawhub.ai/github.com URLs or clawhub:/github: refs. Shows a trust preview (files, translated requirements, hub verdict, security warnings) and requires consent; hub-flagged malicious skills are refused with no override. Re-running install on the same ref re-checks and re-pins — that is the update path.', examples: [{ title: 'Install from a pasted ClawHub URL', code: 'bakin skills install https://clawhub.ai/steipete/skills/weather', test: 'illustrative', reason: 'Network + mutates local state.' }, { title: 'Install a Pi skill from GitHub', code: 'bakin skills install github:badlogic/pi-skills#brave-search', test: 'illustrative', reason: 'Network + mutates local state.' }] }),
  cli({ name: 'skills list', usage: 'bakin skills list [--json]', group: 'Agents and packages', summary: 'List managed and unmanaged skills.', description: 'Lists Bakin-managed skills (hub-installed and pack-installed) with version and source, plus unmanaged skills discovered in the active runtime.', examples: [{ title: 'List skills', code: 'bakin skills list', test: 'illustrative', reason: 'Depends on local state.' }] }),
  cli({ name: 'skills remove', usage: 'bakin skills remove <name>', group: 'Agents and packages', summary: 'Remove an installed skill.', description: 'Uninstalls a managed skill by its bare name (unprojects from the runtime, updates the lockfile).', examples: [{ title: 'Remove a skill', code: 'bakin skills remove weather', test: 'illustrative', reason: 'Mutates local state.' }] }),
  cli({ name: 'skills map', usage: 'bakin skills map <name> [--yes]', group: 'Agents and packages', summary: 'Map unrecognized skill requirements via an agent.', description: 'Dispatches an agent to read an installed hub skill and propose a requirements mapping (keys, binaries). The proposal is mechanically verified (only literally-present names survive; secret slots are core-minted) and shown as a diff for approval before the manifest is amended.', examples: [{ title: 'Map requirements', code: 'bakin skills map some-skill', test: 'illustrative', reason: 'Dispatches an LLM turn.' }] }),
  ...
  cli({ name: 'models', usage: 'bakin models <plan [--apply]|restore <snapshot.json>> [--json]', group: 'Setup and config', summary: 'Recommended model plan; restore a Reset snapshot.', description: 'plan prints the recommended two-lane model plan (agent model + background chores) with reasons and the changes that reach it; --apply submits them through the validated selections write path (never automatic). restore diffs a Models-page Reset snapshot (plugin-settings/models/snapshots/*.json) against the CURRENT selections and applies the difference under the current revision — undoes a Reset even after later edits.', examples: [{ title: 'Show the recommended plan', code: 'bakin models plan', test: 'illustrative', reason: 'Needs a running server.' }, { title: 'Apply it', code: 'bakin models plan --apply', test: 'illustrative', reason: 'Mutates model configuration.' }, { title: 'Undo the last Reset', code: 'bakin models restore ~/.bakin/plugin-settings/models/snapshots/<file>.json', test: 'illustrative', reason: 'Mutates model configuration.' }] }),

  cli({ name: 'spend', usage: 'bakin spend [--window 24h|7d|30d|all] [--json]', group: 'Budget and spend', summary: 'Estimated agent spend + budget utilization.', description: 'Lane-split spend (metered dollars vs subscription tokens — plan quota carries no dollar figure), per-rule budget utilization, on-pace projections with their observed-days basis, unattributed share, per-agent/per-model rollups, and the kill-switch state. Same engine the dispatch gate enforces, so the numbers match the Spend page.', examples: [{ title: 'Current spend vs caps', code: 'bakin spend', test: 'illustrative', reason: 'Requires running server data.' }] }),
  cli({ name: 'budget', usage: 'bakin budget [show|set|rm|pause|resume|incidents] ... (set: --monthly N [--daily N] [--at-cap wait|pause] [--scope global|agent|provider|model --id <scopeId>] [--lane metered|subscription]; token caps accept k/M suffixes)', group: 'Budget and spend', summary: 'Manage opt-in spend limits + the dispatch kill switch.', description: 'Shows or edits spend limits (scope global|agent|provider × lane metered|subscription; metered limits are whole USD, subscription limits are tokens; every limit notifies at 50/75/90% and when reached — at the line, work waits for the next period or pauses until you raise/resume), shows milestone state, pauses/resumes ALL dispatch (kill switch), and lists/resolves budget incidents (raise a limit & resume, acknowledge, or resume a pause-mode hold — refused while still over).', examples: [{ title: 'Limit metered spend to $100/month', code: 'bakin budget set --monthly 100', test: 'illustrative', reason: 'Mutates spend settings.' }, { title: 'Pause matching work at the line instead of waiting', code: 'bakin budget set --monthly 100 --at-cap pause', test: 'illustrative', reason: 'Mutates spend settings.' }, { title: 'Break-glass pause', code: 'bakin budget pause', test: 'illustrative', reason: 'Pauses all dispatch.' }, { title: 'Open incidents', code: 'bakin budget incidents', test: 'illustrative', reason: 'Requires running server data.' }] }),

/**
 * `whiskit-artifacts.json` — the per-release catalog a plugin repo attaches to
...
export const INDEX_FILENAME = 'whiskit-artifacts.json'
...
export function readArtifactsIndex(path: string): ArtifactsIndex {
  ...
  try {
    ...
    throw new Error(
      `whiskit-artifacts.json at ${path} is not valid JSON: ${err instanceof Error ? err.message : String(err)}`,
    )
  }

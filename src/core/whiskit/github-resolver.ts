 ...
 * the right release's `whiskit-artifacts.json`, plus the pluginId derived from
 * the subpath:
...
import { httpIndexResolver, type WhiskitArtifactResolver } from './resolver'
...
export function githubArtifactSource(source: string): GithubArtifactSource {
  ...
  if (!ownerRepo) {
    throw new Error(`Whiskit: cannot derive owner/repo from source "${source}"`)
  }
  if (!parsed.subpath) {
    throw new Error(
      `Whiskit: a published plugin source needs a "#subpath" (e.g. github:owner/repo#plugins/messaging)`,
    )
  }

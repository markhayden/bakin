...
function buildContentTaskDescription(
  ...
  const channels = Array.isArray(item.channels) && item.channels.length > 0
    ? item.channels.filter((channel): channel is string => typeof channel === 'string' && channel.length > 0).join(', ')
    : 'general'
...
3. If this content does NOT need image/video, or when you are re-dispatched after assets complete:
   ...
   - IMPORTANT: Use the bare filename returned by the asset save tool (e.g. "20260416-trainer-workout-a1b2c3d4.png"), NOT a path. Filenames are globally unique and stable across retype/relink.

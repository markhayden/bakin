...
export async function POST(
  ...
  if (result.ok) {
    appendAudit(`agent.${action}`, 'roscoe', { agent })
  } else {
    appendAudit(`agent.${action}.failed`, 'roscoe', { agent, error: 'error' in result ? result.error : 'note' in result ? result.note : 'unknown' })
  }

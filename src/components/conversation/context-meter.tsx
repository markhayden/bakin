...
export function ContextMeter({ stats }: { stats?: ContextMeterStats | null }) {
  ...
  // Post-compaction gap (or stale store): tokens unknown. With a known
  // compaction we can say WHY it's unknown; without one there's nothing
  // honest to render at all.
  if (tokens === null) {
    if (!lastCompaction?.at) return null
    return (
      <span data-context-meter title={TOOLTIP} className="inline-flex items-center gap-1.5 text-muted-foreground/80">
  ...
  if (contextWindow === null) {
    return (
      <span data-context-meter title={TOOLTIP} className="text-muted-foreground/80">
  ...
  const tickPercent =
    compactionThreshold !== null && compactionThreshold > 0 && compactionThreshold <= contextWindow
      ? Math.round((compactionThreshold / contextWindow) * 100)
      : null

 ...
 */
import type { ChatChunk as RuntimeChatChunk, RuntimeToolActivity, ChatTextFormat } from '@bakin/core/adapters/runtime'
...
export type TurnItem =
  | { type: 'text'; format: ChatTextFormat; content: string }
...
/** Fold one structured tool activity (live chunk or persisted row) into the builder. */
function addToolActivity(builder: AgentTurnBuilder, data: RuntimeToolActivity): void {
  if (!data.toolName) return
  const status: ConversationToolCall['status'] =
    data.phase === 'result' ? settledStatus(data.status) : 'running'

  // Locate the call this event belongs to.
  let existing: ConversationToolCall | undefined
  if (data.callId) {
    existing = builder.byCallId.get(data.callId)
  } else if (data.phase === 'result') {
    // callId-less result: close the most recent RUNNING call of this tool.
...
function addText(builder: AgentTurnBuilder, content: string, format: ChatTextFormat): void {
  if (!content) return
  const last = builder.items[builder.items.length - 1]
...
/** Fold the in-flight turn's raw chunks into an agent-turn builder. */
function foldLiveChunks(chunks: readonly RuntimeChatChunk[]): AgentTurnBuilder {
...
export function foldConversation(
  ...
  for (const row of messages) {
    ...
    // adjacency until the next user row).

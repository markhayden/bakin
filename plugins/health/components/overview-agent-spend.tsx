...
export function OverviewAgentSpend({
  ...
  const rankedRows = [...(history?.byAgent ?? [])]
    .sort((left, right) => right.tokens.total - left.tokens.total || left.agent.localeCompare(right.agent))
  const rows = rankedRows.slice(0, 4)
  const total = history?.byAgent.length
    ? history.byAgent.reduce((sum, row) => sum + row.tokens.total, 0)
    : history?.byDay.reduce((sum, row) => sum + row.tokens.total, 0) ?? 0
  ...
  const cost = history ? reportedCost(history) : null
        ...
        <>
          <div className="mt-4 flex flex-wrap items-end gap-x-6 gap-y-2">
            <div>
              ...
              <span className="text-xs text-muted-foreground">Observed transcript traffic</span>
            </div>
            <div>
              <strong className="block text-lg font-semibold tabular-nums text-foreground">
                {cost === null ? 'Cost unavailable' : `${formatRuntimeCost(cost.usd)}${cost.complete ? '' : '+'}`}
              </strong>
              <span className="text-xs text-muted-foreground">
                {cost?.complete === false ? 'Partial runtime-reported cost' : 'Runtime-reported cost'}
              </span>
            </div>
          </div>

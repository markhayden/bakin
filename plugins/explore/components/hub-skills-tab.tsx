...
/**
 * Hub Skills tab (#687, D7): paste a clawhub.ai / github.com link →
 * trust-gate preview in a modal (files, requirements, verdict, security
 * warnings) → consent → install. Below the box: the installed hub/pack
 * skills with remove, and unmanaged runtime skills for context. The whole
 * install/remove flow lives in modals — nothing mutates inline.
 */
  ...
  return (
    <Dialog open onOpenChange={(open) => { if (!open) onCancel() }}>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle>
            Install {preview.skillName} v{preview.version}?
          </DialogTitle>
          <DialogDescription>
            {preview.description ?? 'No description.'}
            <span className="mt-1 block font-mono text-xs">{preview.ref}</span>
          </DialogDescription>
        </DialogHeader>
        ...
        <details className="text-sm">
          <summary className="cursor-pointer text-muted-foreground">Files ({preview.files.length})</summary>
...
export function HubSkillsTab() {
  const { data, loading, error, refresh } = useJsonFetch<SkillsListResponse>('/api/skills')
  ...
  const runInstall = async (p: SkillPreviewWire) => {
    ...
    try {
      ...
      setDialogError(body.refused ? `Refused: ${body.error}` : body.error ?? `HTTP ${res.status}`)
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-lg border p-4">
        <div className="mb-1 text-sm font-medium">Install from a hub</div>
        <div className="mb-3 text-xs text-muted-foreground">
          Browse clawhub.ai or a GitHub skills repo, copy the page link, paste it here.
          Every install shows a full preview first; nothing runs at install time.
        </div>
      ...
      {error && <div className="text-sm text-red-500">Couldn&apos;t load skills: {String(error)}</div>}

      {data && data.managed.length === 0 && data.unmanaged.length === 0 && !loading && (
        <EmptyState
          title="No skills installed yet"
          description="Paste a link above to install your first skill — it lands on whichever runtime is active."
        />
      )}

      {data && data.managed.length > 0 && (
        <div>
          <div className="mb-2 text-sm font-medium">Installed skills</div>
          <div className="divide-y rounded-lg border">
            {data.managed.map((row) => (
              <div key={row.packageId + row.skillName} className="flex items-center justify-between gap-2 p-3">
                <div className="min-w-0">
                  <div className="text-sm font-medium">
                    ...
                    <span className="rounded bg-muted px-1.5 py-0.5 text-[10px] uppercase text-muted-foreground">{row.hub ? 'hub' : 'pack'}</span>
                  </div>
      ...
      {data && data.unmanaged.length > 0 && (
        <div>
          <div className="mb-2 text-sm font-medium text-muted-foreground">Unmanaged runtime skills</div>
          <div className="text-xs text-muted-foreground">
            {data.unmanaged.map((u) => u.name).join(' · ')}
          </div>
        </div>
      )}

...
- **Budget gate** (`budgetGate`, before each `claimDispatchRun`): reads `models.getBudgetPolicy` + ledger spend (day/month, global/agent) and decides allow/warn/defer (`src/core/budget.ts`). **defer** → don't claim, leave the task in todo (resumes when the window rolls over or the cap is raised — never pauses the agent). **Fail-closed**: an unreadable spend ledger defers. warn/defer audits debounce per window.

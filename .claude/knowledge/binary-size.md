...
| sharp | 213 KB JS | **External since #889** | Image downscale/dimensions/thumbnails. Natives never survived `--compile` anyway (JS bundled dead); now in `EXTERNAL_NATIVE_MEDIA` (`scripts/build-binary.ts`) so the binary's `import('sharp')` fails fast and the loader falls back to the media store (`~/.bakin/media/`, `bakin install media`). Safe to externalize UNLIKE react-devtools-core below: sharp is only ever DYNAMICALLY imported inside `loadSharp()`'s try/catch (playwright class, not eager-static ink class) — pinned by `tests/integration/media/compiled-sharp-store.test.ts`. Deep ref: `.claude/knowledge/media-pipeline.md`. |

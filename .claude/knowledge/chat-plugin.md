...
- **Per-turn usage is VISIBLE in the thread (#733).** `GET /chats/:id` joins `run_costs` by the `chat:<id>:turn:` runId prefix (`lib/usage.ts` over the ledger's `listRunCostsByPrefix` — a read-time display join over the ONE spend engine, no new persistence) into `usage` (turnId → tokens/cache/model/lane) + `usageTotals`. Unit-per-lane: `costUsd` exists ONLY on metered-lane rows with a recorded cost — subscription/unknown lanes never show dollars. Unknown is ABSENT (no footer/chip), never zero; a ledger failure serves the transcript without usage, never a 500. UI: muted footer under each settled turn (`14.2k in / 890 out · $0.03 · claude-sonnet-5`) + the header's chip in PLAIN WORDS leading with the **context monitor** (`context 49.5k / 200k (25%) · 500.4k tokens · $0.41` — the Σ sigil was dropped on live feedback). Context = the LAST settled turn's provider-reported prompt size (input + cacheRead; drops after a runtime compaction) over the model's NUMERIC window from the `models.getAvailableModels` hook — window unknown → tokens only, never parsed from display strings (D1 amendment); both refresh on the settle refetch, so they tick the moment a reply finishes. While a reply streams, the STREAMING TURN shows a ~-labeled output-so-far estimate far right of the shimmer row (`~320 out…`, chars÷4, kit `AgentTurn.liveOutEstimate` via `Conversation.liveOutEstimate`) that the real footer replaces at settle — the ONLY estimate anywhere, never blended into recorded totals or the header chip (input tokens are unknowable mid-turn; no input estimate exists). NO percentage window gauge by decision (D1, spec `chat-turn-usage.md`) — per-turn `in` tokens ARE the honest context-growth signal. Historical turns light up retroactively.

...
- **Per-turn usage is VISIBLE in the thread (#733).** `GET /chats/:id` joins `run_costs` by the `chat:<id>:turn:` runId prefix (`lib/usage.ts` over the ledger's `listRunCostsByPrefix` — a read-time display join over the ONE spend engine, no new persistence) into `usage` (turnId → tokens/cache/model/lane) + `usageTotals`. Unit-per-lane: `costUsd` exists ONLY on metered-lane rows with a recorded cost — subscription/unknown lanes never show dollars. Unknown is ABSENT (no footer/chip), never zero; a ledger failure serves the transcript without usage, never a 500. UI: muted footer under each settled turn (`14.2k in / 890 out · $0.03 · claude-sonnet-5`) + the header's Σ chip; both refresh on the settle refetch, so they tick the moment a reply finishes. NO percentage window gauge by decision (D1, spec `chat-turn-usage.md`) — per-turn `in` tokens ARE the honest context-growth signal. Historical turns light up retroactively.

...
Beacon uses the filesystem as its database. All state is stored as markdown files, JSON files, and JSONL logs in a content directory (`~/.beacon/`). There is no database, no ORM, no migrations in the traditional sense. The filesystem IS the data layer.

## Content Directory

### Resolution (`src/core/content-dir.ts`)
Priority order:
...
### Well-known paths (`getBeaconPaths()`)
Returns a `BeaconPaths` object with absolute paths:

| Key | Path | Purpose |
|-----|------|---------|
| `home` | `~/.beacon/` | Content root |
...
| `settings` | `.beacon/settings.json` | Runtime settings |

### Initialization (`initBeaconHome()`)
Called by `beacon init` or first run. Creates full directory structure including:
- All asset type directories with `_unlinked/` and `library/` subdirs
- `.beacon/` config directory
...
### Settings (`.beacon/settings.json`)
`BeaconSettings` interface defined in `src/core/settings.ts`. Deep-merged with hard defaults at load time. Cached with `resetSettingsCache()` for invalidation.
...
- Configured in `BeaconSettings.antfly` (enabled, url, auth)

## Key Files

| File | Purpose |
|------|---------|
| `src/core/content-dir.ts` | Content directory resolution, path constants, init |

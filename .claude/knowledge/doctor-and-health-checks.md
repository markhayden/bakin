...
The Models `routing` check (`models.routing`, `plugins/models/lib/health-checks.ts`) warns on unrouted recommended system work classes (with per-class 7d spend evidence), on standing thinking clamps, and on premium-tier models observed on cheap-recommended classes. Its `apply-recommended-routes` repair applies the same recommendation engine the Routing tab's `GET /routing/recommend` preview uses — through the selections mutator, never a direct settings write.

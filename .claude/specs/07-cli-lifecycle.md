...
Agent workspaces:
  ~/.openclaw/workspaces/main-operator/AGENTS.md
...
MCP configuration:
  ~/.openclaw/workspaces/main-operator/mcp.json
...
Managed Files:
  ...
  [WARN] ~/.openclaw/workspaces/chef/AGENTS.md — block 'agent-tools' modified manually
...
MCP Configuration:
  ...
  [WARN] Agent 'chef' has 2 unknown MCP servers (user-added, not managed)

Recommendations:
  1. Run `bakin sync` to fix 1 outdated tool list
  2. Remove orphaned skill: ~/.openclaw/workspaces/main-operator/skills/analytics-report.md
  3. Review deprecated tool reference in main-operator's user section

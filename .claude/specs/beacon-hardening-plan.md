...
**Critical bug:** The catch-all at `src/app/api/plugins/[pluginId]/[...path]/route.ts` creates a no-op `MCEventBus` (`broadcast = () => {}`), so plugin API routes cannot broadcast SSE events.
...
tests/
  ...
  plugins/
    contract.test.ts        — load all plugins, verify MCPlugin interface
...
```
beacon status                     — system health, agents, dispatch timer
...
Every plugin gets a `beacon-plugin.json`:
...
**Files:** New `plugins/*/beacon-plugin.json` (5 files), `/src/lib/plugin-types.ts` (add PluginManifest), `/src/lib/plugin-registry.ts` (manifest validation), `/mc.config.ts` (support manifest-based loading)
...
**Files:** `/src/lib/plugin-registry.ts` (migration runner), `/src/lib/plugin-types.ts` (add migrations to MCPlugin)
...
- **Phase 4:** `beacon status` works, plugins have manifests, SSE reconnects after disconnect

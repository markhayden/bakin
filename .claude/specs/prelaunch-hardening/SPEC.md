...
- OQ2. Whether `chat` frames alone suffice for streaming text (with `agent` frames only for tool/lifecycle), or both text sources need merging — resolve empirically in PR 1a's first spike; prefer the single cooked `chat` stream if sufficient (less merge logic).

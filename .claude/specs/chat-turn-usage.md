...
| # | Decision | Choice |
|---|---|---|
| D1 | Context-window display | **Absolute tokens only.** Per-turn `in` tokens are the honest window-growth signal. NO percentage gauge: the catalog's context field is a display string, and session runtimes (caching/compaction) make provider context ≠ transcript tokens — a gauge would be confidently wrong. Revisit only if a runtime ever reports true context size. **A context-monitor amendment was BUILT AND REVERTED 2026-07-25** (live screenshot showed 109%): run_costs usage is SUMMED ACROSS the tool-loop's internal requests — `cache_read_tokens` re-counts the same prefix once per tool call (verified in prod rows: `total = input + cacheRead + output`, e.g. 22.6k + 273.9k on a 272k-window model) — so NO honest per-turn context snapshot can be derived from billing aggregates, with any arithmetic. The footers' `in/out` are billing sums (honest as spend), not prompt sizes. A real context monitor needs the RUNTIME to report session context — DELIVERED by #737 (`sessions.contextStats` + the chat compaction bar; spec `chat-context-instruments.md`). This post-mortem remains the standing ban on billing-derived context. |

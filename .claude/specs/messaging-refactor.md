...
- **Existing frontmatter with legacy values** (`agent: 'chef'`, `contentType: 'recipe'`, etc.): parses fine because types widen to strings. Renders degraded (raw id, no emoji). Maintainer wipes `~/.bakin/messaging/` manually before deploying this change — no migration code to maintain.
- **In-flight workflow instances referencing `agent: 'chef'`:** unchanged — workflows reference agent ids as strings already. No impact.
...
- **Regression guard:** fixture test — a `messaging.md` with `agent: 'chef'` and `contentType: 'recipe'` parses, and the client-side lookup renders sensibly whether the team hook mock returns an agent or `null`.
...
- [ ] No string literals like `'chef' | 'explorer' | 'trainer' | 'coach'` or `'recipe' | 'tip' | ...` anywhere under `plugins/messaging/`

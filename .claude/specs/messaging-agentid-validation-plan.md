...
- `tests/plugins/messaging/prompt-builder.test.ts`
  ...
  - Update `opts()` helper:
    ...
    ```
  - Tests that previously asserted persona inclusion by writing a persona file and checking the output now pass `persona: 'Basil is a chef...'` directly and assert the string appears in the prompt. Much cleaner.
...
- Mock `team.getAgentIds` via the ctx's HookRegistry by registering a handler that returns a known roster (e.g., `['basil']`).
...
5. **shape-valid-but-unknown (`ghost`)** — roster returns `['basil']`, `ghost` not in it → 400.
6. **happy path (`basil`)** — roster returns `['basil']`, a `personas/basil.md` fixture exists → 200, response body has the expected brainstorm shape (just assert status + one top-level key, don't over-assert).
7. **happy path with no persona file** — roster returns `['basil']`, no persona file → 200 (brainstorm proceeds with empty persona).
8. **roster unavailable (hook throws)** — shape guard alone passes `basil` through → 200.
...
- Active runtime boundary (`ctx.runtime` or `src/core/app-services`) → runtime messaging returns a canned response so the happy path doesn't try to reach a real provider.

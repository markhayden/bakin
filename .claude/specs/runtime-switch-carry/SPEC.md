...
| D3 | **Dry-run is strictly zero-write.** `POST /api/runtime/switch { target, dryRun: true }` / `bakin runtime use <t> --dry-run`. No settings flip, no backup file, no deprovision, no writes to the target's home. |

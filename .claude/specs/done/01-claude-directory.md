...
This is about the **development tool** (Claude Code), not the **application** (Bakin). Bakin's runtime agents (Pixel, Chef, etc.) are application data — they'll live in the app's plugin/package structure, not in `.claude/`.
...
plugin imports       → import { readProject } from '@bakin/projects/lib/parser'
...
- Entry point exports an object implementing `BakinPlugin` interface (currently `BakinPlugin`)
...
- Key files: `src/lib/plugin-types.ts`, `src/lib/plugin-registry.ts`, `bakin.config.ts` (currently `bakin.config.ts`)

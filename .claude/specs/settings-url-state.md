...
| D4 | Producers | **All six existing `/settings` links retargeted in this PR** (table below). Hand-written hrefs, matching the existing `/models?tab=routing` / `/health?tab=activity` convention — no URL-builder abstraction. The structured `{ kind: 'setting', id }` incident resource stays as-is (not a routing mechanism). |
...
4. The runtime Capabilities tab uses TanStack `Link` (host-internal, allowed); only its `to` changes.

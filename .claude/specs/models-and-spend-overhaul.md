...
1. Pi "cached roster" (#907 item 5) — **RESOLVED (T1.11): no cache.** Pinned by `tests/integration/pi/model-change-no-restart.test.ts`: a model change via `agents.update` is what the very next turn sends to the (fake) provider, no `restart()` in between. Pi re-reads `bakin-agents.json` per turn and refreshes its model registry per catalog read. What margo saw (2026-09-20) was most likely the enrichment queue's already-started pump reusing its engine choice, not an adapter cache; the dirty-marker banner on Pi was a false alarm. Live confirmation on the Pi box is part of Checkpoint A.

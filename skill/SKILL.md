...
Your MCP server is `bakin-<agent>`. Roscoe/main uses `bakin-main`.
...
```bash
mcporter call bakin-main.bakin_exec_tasks_create --args '{"title":"Write todays story","assignee":"nemo","description":"Write a short story and post the result back to #danger-zone.","skipWorkflowReason":"one-off chat request"}'
```

...
`bakin models plan` prints the current lanes, the recommended plan with its reasons, and the changes that would reach it; `bakin models plan --apply` applies them (the only way the CLI ever changes a model, and only on that flag). `bakin models restore <snapshot.json>` undoes a Reset. `bakin models pending` lists writes the runtime has not confirmed; if one is marked *conflict* (the runtime's config changed outside Bakin while the write was pending) that setting stays locked until you decide — `bakin models pending --ack <document>` keeps what is on disk and unlocks it. `bakin onboard` shows the recommended plan on a fresh install and applies it only when you confirm (or pass `--yes`); it never changes a plan you already have — a dead lane on an existing install becomes a warning that points you back here. A default model that works is never moved; if the default can't run here (no credentials, retired), `--yes` repairs it to the strongest model that can — on a dead default nothing dispatches, and the output names what it chose.

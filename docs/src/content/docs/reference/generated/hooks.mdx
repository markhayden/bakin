...
<div class="hook-card-list">
<HookCard id="messaging-sweep-run" name="messaging.sweep.run" label="Run messaging content sweep" summary="Run messaging content sweep" source="bakin-bits-official/plugins/messaging/index.ts:551" badges={["rpc"]}>
```ts frame="terminal"
const result = await ctx.hooks.invoke(
  'messaging.sweep.run',
  {},

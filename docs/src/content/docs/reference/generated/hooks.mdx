...
<div class="hook-card-list">
<HookCard id="chat-resolveactiveturn" name="chat.resolveActiveTurn" label="chat.resolveActiveTurn" source="plugins/chat/index.ts:45">
```ts frame="terminal"
const result = await ctx.hooks.invoke(
...
<div class="hook-card-list">
<HookCard id="health-getcheck" name="health.getCheck" label="Get a health check." summary="Returns canonical metadata for one registered Health check by stable id without executing it." source="plugins/health/index.ts:846" badges={["rpc"]}>
```ts frame="terminal"
const result = await ctx.hooks.invoke(
  'health.getCheck',
  {
...
<HookCard id="health-list" name="health.list" label="List health checks." summary="Returns canonical metadata for registered Health checks without executing them." source="plugins/health/index.ts:845" badges={["rpc"]}>
```ts frame="terminal"
const result = await ctx.hooks.invoke(
  'health.list',
  {},

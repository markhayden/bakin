...
<div class="hook-card-list">
<HookCard id="health-getcheck" name="health.getCheck" label="Get a health check." summary="Returns canonical metadata for one registered Health check by stable id without executing it." source="plugins/health/index.ts:847" badges={["rpc"]}>
```ts frame="terminal"
const result = await ctx.hooks.invoke(
  'health.getCheck',
  {
...
<HookCard id="health-list" name="health.list" label="List health checks." summary="Returns canonical metadata for registered Health checks without executing them." source="plugins/health/index.ts:846" badges={["rpc"]}>
```ts frame="terminal"
const result = await ctx.hooks.invoke(
  'health.list',
  {},
...
<HookCard id="models-getbudgetpolicy" name="models.getBudgetPolicy" label="Get budget policy." summary="Returns the spend-cap rule list that dispatch consults before each turn. Use it to read the current budget limits." source="plugins/models/lib/register-hooks.ts:163" badges={["rpc"]}>
```ts frame="terminal"
const result = await ctx.hooks.invoke(

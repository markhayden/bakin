...
<CliCommandCard id="agents-remove" name="agents remove" summary="Remove an agent package." description="Removes an installed agent package and optionally deletes the runtime agent.">
```sh frame="terminal"
...
<CliCommandCard id="setup-service" name="setup service" summary="Install or remove autostart service integration." description="Installs or removes a macOS LaunchAgent or Linux user systemd service that runs `bakin serve` on login. Manual startup remains the default until this command is run.">
```sh frame="terminal"

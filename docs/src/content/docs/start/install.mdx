...
Each release publishes platform archives plus a `checksums.txt` with SHA-256 sums for the archives.

<div class="table-light-fit table-label">

| Platform | Asset |
| --- | --- |
| macOS (Apple Silicon) | `bakin-darwin-arm64.tar.gz` |
| Linux x64 | `bakin-linux-x64.tar.gz` |
| Linux arm64 | `bakin-linux-arm64.tar.gz` |
...
Pull the archive for your platform from the [GitHub releases page](https://github.com/markhayden/bakin/releases), check it against `checksums.txt`, extract the `bakin` executable, and drop it on your `PATH`. Same outcome as the installer, just more steps.

...
The canonical template is the in-repo reference plugin ([`examples/reference-plugin/`](https://github.com/markhayden/bakin/tree/main/examples/reference-plugin)) — a small bookmarks manager that exercises every surface below with the smallest honest example of each, written entirely against `@makinbakin/sdk/*` and install-gated by CI so it can never drift from the host. It is mirrored to [`markhayden/bakin-plugin-starter`](https://github.com/markhayden/bakin-plugin-starter) on stable releases as a forkable starting point.
...
- [Server Contracts](/docs/extending/plugins/server-contracts/): routes, tools, hooks, storage, health checks, skills, workflows, and cleanup
- [Realtime Events](/docs/extending/plugins/realtime/): live UI updates over the shared SSE stream
- [Search](/docs/extending/plugins/search/): making plugin content searchable

...
- [ ] CHECKPOINT A: full suite green; zero-unattributed parity pinned

## Phase 2 — Rules, incidents, modes
- [ ] T6 `feat(core): BudgetRule policy + rule-based evaluator + gate provider resolution` (incl. one-shot settings migration; routing hoisted above gate)
...
- [ ] T10 `feat(core): dispatch kill switch (settings.dispatch.paused)`
- [ ] CHECKPOINT B: `/verify` — seed cap, breach, assert defer + incident + audit

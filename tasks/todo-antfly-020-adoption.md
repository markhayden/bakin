...
- [ ] **T1: Stage binary + empirical probes** (S)
...
- [ ] **T2: Ladder R1–R3 + empty-dir boot** (S)
...
- [ ] **T4: #382 poison-read + #386 hot-queue drop repros** (S)
...
- [ ] **T6: Bare reindex crucible + rc.18 baseline** (M)
...
- [ ] Every rung has a PASS/FAIL verdict; **commit 1**
  `test(search): record antfly 0.2.0 evaluation evidence` on branch
  `feat/antfly-0.2.0-adoption`.
- [ ] FAIL anywhere → STOP: counter-evidence + upstream tickets, effort ends.

## Phase 1 — Repin

- [ ] **T8: The repin commit** (M)
...
- [ ] **T9: Full suites vs 0.2.0 binary** (S)
...
- [ ] test:ci + typecheck + integration green; system works with all workarounds.

## Phase 2 — De-Hardening (one commit per candidate)

- [ ] **T10: Pin-flip census** (XS) — run workaround-regressions vs 0.2.0;
...
- [ ] **T11: composeFtsWithFilters removal** (S) — if filter_query fixed.
  PIN→GUARD, delete workaround in `translate.ts`. Verify: suite + conformance.
- [ ] **T12: mapIndexStatuses `!runtime` idle block removal** (S) — empty-table
  flags now honest. PIN→GUARD.
- [ ] **T13: EMBED_SAFE_RE + thumbs-first removal** (M) — if undecodable media
  no longer poisons batches; includes rc.20 ReadUnavailable-heal pin rework.
- [ ] **T14: read-unavailable-storm wedge pattern removal** (S) — if #382 holds
  (T4); `engine-status.ts` + its tests.
- [ ] **T15: Serialization-gate removal** (M) — LAST. Requires: T3 clean +
...
- [ ] **T16: #319 idle-override resolution** (S) — remove ONLY if T7 verdict is
  reproduced-then-clean; else mark deferred-to-burn-in in code comment +
  evidence, revisit at T23.
- [ ] Also from census: order_by/sort enablement, /documents body — fold into
  the list above as their pins dictate (same one-commit rule).

### CHECKPOINT DEHARDEN
- [ ] Zero stale pins; every candidate removed/retained/deferred; test:ci green.

## Phase 3 — Docs + Survey

- [ ] **T17: Docs sweep** (M) — **commit 4**: CLAUDE.md Search bullet;
...
- [ ] **T18: New-feature deep dive** (M) — AFB backup/restore, /ml/v1,
...
- [ ] **T19: Chaos drills** (S) — all five vs 0.2.0; knowledge doc updated.
- [ ] **T20: Rig e2e + full-stack reindex crucible** (M) — isolated instance,
  seeded corpus, sustained query load during `bakin reindex --force`;
  availability + honest degrades + latencies recorded.

### CHECKPOINT PR (human)
- [ ] Branch pushed, PR open, CI green → merge decision.

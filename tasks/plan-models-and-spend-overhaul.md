...
## T2.5 — spend ownership series (L — landed as FOUR commits; restructured 2026-09-22)
**Why restructured:** `tests/architecture/plugin-boundaries.test.ts` forbids ANY `plugins/<a>` → `plugins/<b>` import with no allowlist, so "Models still imports the moved files via `@bakin/spend/*`" was impossible. A file can only move when its OWNER moves with it. Landed:
1. `refactor(core)`: `plugins/health/lib/usage-history-timer.ts` → `src/core/usage-history-timer.ts` (health still starts/stops it); `repairTargetSelection` (health copy + tasks private copy) → `@bakin/core/health/repair-support`.
2. `refactor(spend)`: `plugins/health/lib/system-checks/budget.ts` → `plugins/spend/lib/health-checks.ts`; spend registers check `spend.budget` + both repairs (`spend-evidence-refresh-pricing`, `accept-unattributed-history`); health drops them; `affectedCheckIds` now the namespaced id (bare `'budget'` never matched). `@bakin/spend` tsconfig alias.
3. `refactor(models)`: PR #500 legacy budget migration deleted (live `models.json` is v2; activate one-shot + two migrate-on-read guards + test removed).
4. `refactor(spend)`: Spend tab UI → `plugins/spend/components/{spend-page,overview-tab,limits-tab,incident-banners,spend-breakdown,spend-budget-controls,spend-utils,use-spend-data}`; wire types in `plugins/spend/types.ts`; roster via `useAgentList`; `SOURCE_PLUGIN = 'models'` is the ONE constant T2.7 flips; every `/models?tab=spend` link → `/spend`; `pluginFetchJson` added to `@makinbakin/sdk/utils` (models' hook delegates). RTL smoke: `tests/plugins/spend/spend-page.test.tsx`.
- **Still in Models until T2.7 (they read Models' settings store):** `lib/billing.ts`, `lib/budget-routes.ts`, budget parts of `lib/route-schemas.ts`, the five `models.*` budget/billing hooks, `types.ts` budget/billing keys. T2.7 `git mv`s them into `plugins/spend/lib/` as part of the cutover.
- Verify: `bun run test && bun run check:cycles && bun run ui:conformance --quick` — green at each commit.

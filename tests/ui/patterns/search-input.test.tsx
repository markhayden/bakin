...
describe('canonical search input', () => {
  it('expands inside a reserved slot on focus and collapses without losing the query', () => {
    ...
    expect(control.style.getPropertyValue('--bakin-search-input-inline-size')).toBe('min(100%, 14rem)')

    fireEvent.focus(input)
    expect(control.dataset.state).toBe('focused')
    expect(control.style.getPropertyValue('--bakin-search-input-inline-size')).toBe('100%')
    ...
    expect(control.style.getPropertyValue('--bakin-search-input-inline-size')).toContain('ch')

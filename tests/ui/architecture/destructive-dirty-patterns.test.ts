...
describe('canonical destructive and dirty-state patterns', () => {
  ...
  it('documents retry, mobile ordering, and the existing routing exception', () => {
    ...
    expect(confirmStory).toContain("title: 'Components/Feedback/ConfirmDialog'")
    expect(dangerStory).toContain("title: 'Components/Feedback/DangerZone'")
    expect(saveBarStory).toContain("title: 'Components/Forms/SaveBar'")
    expect(exitStory).toContain("title: 'Components/Forms/UnsavedChangesDialog'")

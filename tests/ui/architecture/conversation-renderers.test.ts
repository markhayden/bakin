...
describe('focused conversation renderers', () => {
  ...
  it('publishes stream plumbing from the focused SDK layer and keeps root modules as adapters', () => {
    const sse = read('packages/sdk/src/conversation/sse.ts')
    const stream = read('packages/sdk/src/conversation/use-conversation-stream.ts')
    const focused = read('packages/sdk/src/conversation/index.ts')
    const sseAdapter = read('src/components/conversation/sse.ts')
    const streamAdapter = read('src/components/conversation/use-conversation-stream.ts')
    expect(focused).toContain('readConversationSseStream')
    expect(focused).toContain('useConversationStream')
    expect(sse).not.toMatch(/@\/|@bakin\/core|@makinbakin\/sdk/)
    expect(stream).not.toMatch(/@\/|@bakin\/core|@makinbakin\/sdk/)
    expect(sseAdapter).toContain("from '@makinbakin/sdk/conversation'")
    expect(streamAdapter).toContain("from '@makinbakin/sdk/conversation'")
  })
})

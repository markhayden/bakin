/**
 * Whiskit artifact assemble / checksum / safe-extract (Phase 3).
 *
...
const mockDir = join(tmpdir(), `whiskit-artifact-mock-${Date.now()}-${randomUUID()}`)
...
import {
  ...
  ArtifactError,
} from '../../../src/core/whiskit/artifact'
...
function freshDir(): string {
  const d = join(tmpdir(), `whiskit-artifact-test-${Date.now()}-${randomUUID()}`)
...
function seedStaging(): string {
  ...
  writeFileSync(join(staging, '.whiskit', 'build.json'), '{}')
  return staging
}

describe('whiskit artifact assemble + checksum', () => {
...
describe('whiskit artifact safe extraction', () => {

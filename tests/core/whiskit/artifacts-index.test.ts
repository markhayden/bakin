/**
 * whiskit-artifacts.json schema, resolver, and carry-forward merge (Phase 3).
 *
...
const mockDir = join(tmpdir(), `whiskit-index-mock-${Date.now()}-${randomUUID()}`)
...
function freshDir(): string {
  const d = join(tmpdir(), `whiskit-index-test-${Date.now()}-${randomUUID()}`)
...
describe('artifacts index schema + IO', () => {
  it('round-trips through write + read', () => {
    const dir = freshDir()
    const path = join(dir, 'whiskit-artifacts.json')

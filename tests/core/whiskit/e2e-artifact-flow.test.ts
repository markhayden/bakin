/**
 * Whiskit artifact-lane end-to-end (Phases 3+4+6): publish a built plugin,
...
const mockDir = join(tmpdir(), `whiskit-e2e-mock-${Date.now()}-${randomUUID()}`)
...
import { materializeArtifact, WhiskitInstallError } from '../../../src/core/whiskit/consumer-install'
...
describe('Whiskit artifact lane end-to-end', () => {
  it('publish -> serve -> resolve -> download -> verify -> extract', async () => {
    ...
    try {
      ...
      expect(existsSync(join(materialized.stagingDir, '.whiskit', 'build.json'))).toBe(true)
  ...
  it('throws NO_PREBUILT_ARTIFACT for an unknown plugin', async () => {
    ...
    ).rejects.toBeInstanceOf(WhiskitInstallError)
  })
})

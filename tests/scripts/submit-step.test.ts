...
describe('submitStepValidated', () => {
  ...
  it('returns fail when no active step found', async () => {
    ...
    const result = await submitStepValidated('task-123', 'step-1', { caption: 'test' }, 'basil')
  ...
  it('fails on schema validation error', async () => {
    ...
    const result = await submitStepValidated('task-123', 'write-copy', {}, 'basil')
    expect(result.ok).toBe(false)
  ...
  it('catches near-duplicate rejection from server', async () => {
    ...
    const result = await submitStepValidated('task-789', 'step-1', { data: 'same' }, 'basil')
    expect(result.ok).toBe(false)
  ...
  it('catches generic server errors', async () => {
    ...
    const result = await submitStepValidated('task-000', 'step-1', { data: 'x' }, 'basil')
    expect(result.ok).toBe(false)

...
const testDir = vi.hoisted(() => {
  ...
  return join(tmpdir(), `bakin-test-calendar-${Date.now()}`)
})
...
function seedItems(items: CalendarItem[]): void {
  writeFileSync(join(testDir, 'calendar.json'), JSON.stringify(items, null, 2))
}
...
beforeEach(() => {
  // Reset to empty calendar before each test
  seedItems([])
  vi.clearAllMocks()
})
...
describe('Calendar routes', () => {
  ...
  describe('POST /', () => {
    ...
    it('emits audit and activity log', async () => {
      ...
      await callRoute(route, plugin.ctx, {
        body: { title: 'Audit Test', agent: 'coach', scheduledAt: '2026-04-20T08:00:00Z' },
      })
  ...
  describe('DELETE /:itemId', () => {
    ...
    it('deletes an existing item', async () => {
      ...
      // Verify item is gone
      const raw = JSON.parse(readFileSync(join(testDir, 'calendar.json'), 'utf-8'))
...
describe('Calendar exec tools', () => {
  // ── bakin_exec_calendar_list ────────────────────────────────────────────

  describe('bakin_exec_calendar_list', () => {
    it('is registered', () => {
      expect(findTool(plugin.execTools, 'bakin_exec_calendar_list')).toBeDefined()
    })
    ...
    it('filters by agent', async () => {
      seedItems([
        makeItem({ id: 'basil1', agent: 'chef' }),
        makeItem({ id: 'scout1', agent: 'explorer' }),
      ])
      const tool = findTool(plugin.execTools, 'bakin_exec_calendar_list')!
      const result = await callTool(tool, { agent: 'explorer' })
      expect(result.count).toBe(1)
  ...
  // ── bakin_exec_calendar_get ────────────────────────────────────────────

  describe('bakin_exec_calendar_get', () => {
    it('is registered', () => {
      expect(findTool(plugin.execTools, 'bakin_exec_calendar_get')).toBeDefined()
    })

    it('returns a single item with full details', async () => {
      seedItems([makeItem({ id: 'get-1', title: 'Full Details', brief: 'The brief' })])
      const tool = findTool(plugin.execTools, 'bakin_exec_calendar_get')!
  ...
  // ── bakin_exec_calendar_create ─────────────────────────────────────────

  describe('bakin_exec_calendar_create', () => {
    it('is registered', () => {
      expect(findTool(plugin.execTools, 'bakin_exec_calendar_create')).toBeDefined()
    })
    ...
    it('creates a new item with all fields', async () => {
      ...
      expect(item.agent).toBe('trainer')
  ...
  // ── bakin_exec_calendar_update ─────────────────────────────────────────

  describe('bakin_exec_calendar_update', () => {
    it('is registered', () => {
      expect(findTool(plugin.execTools, 'bakin_exec_calendar_update')).toBeDefined()
    })

    it('updates an existing item', async () => {
      seedItems([makeItem({ id: 'tool-upd-1', title: 'Before' })])
      const tool = findTool(plugin.execTools, 'bakin_exec_calendar_update')!
  ...
  // ── bakin_exec_calendar_approve ────────────────────────────────────────

  describe('bakin_exec_calendar_approve', () => {
    it('is registered', () => {
      expect(findTool(plugin.execTools, 'bakin_exec_calendar_approve')).toBeDefined()
    })

    it('approves draft to scheduled', async () => {
      seedItems([makeItem({ id: 'tool-appr-1', status: 'draft' })])
      const tool = findTool(plugin.execTools, 'bakin_exec_calendar_approve')!
  ...
  // ── bakin_exec_calendar_reject ─────────────────────────────────────────

  describe('bakin_exec_calendar_reject', () => {
    it('is registered', () => {
      expect(findTool(plugin.execTools, 'bakin_exec_calendar_reject')).toBeDefined()
    })

    it('rejects a review item back to draft', async () => {
      seedItems([makeItem({ id: 'tool-rej-1', status: 'review', title: 'Review Me' })])
      const tool = findTool(plugin.execTools, 'bakin_exec_calendar_reject')!

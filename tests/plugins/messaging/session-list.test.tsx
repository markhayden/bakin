    ...
    if (url === '/api/plugins/messaging/sessions' && opts?.method === 'POST') {
      return Promise.resolve({
        ...
        json: () => Promise.resolve({
          session: { id: 'new-session-id', agentId: 'chef', title: 'New planning session' },
        }),
      })
    }
...
describe('SessionList', () => {
  ...
  it('shows agent cards in empty state', async () => {
    ...
    expect(screen.getByTestId('agent-card-explorer')).toBeDefined()
    expect(screen.getByTestId('agent-card-trainer')).toBeDefined()
    expect(screen.getByTestId('agent-card-coach')).toBeDefined()
  })
  ...
  it('shows active before completed (sorting)', async () => {
    ...
    // Active sessions should appear before completed in the chef group

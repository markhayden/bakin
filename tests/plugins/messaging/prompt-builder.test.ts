...
describe('buildSystemPrompt', () => {
  it('includes agent name and planning instructions', () => {
    const prompt = buildSystemPrompt('chef', makeSession(), testDir)
    expect(prompt).toContain('You are Chef')
  ...
  it('omits persona section when file is missing', () => {
    const prompt = buildSystemPrompt('explorer', makeSession({ agentId: 'explorer' }), testDir)
    expect(prompt).not.toContain('Your Persona')
    expect(prompt).toContain('Explorer (Connor)')
  })
  ...
  it('includes plan state with proposal statuses', () => {
    const session = makeSession({
      proposals: [
        ...
        {
          id: 'p2', messageId: 'm1', revision: 1, agentId: 'chef',
    ...
    const prompt = buildSystemPrompt('chef', session, testDir)

...
describe('buildSystemPrompt', () => {
  it('uses the agentName from options when provided', () => {
    const prompt = buildSystemPrompt('chef', makeSession(), opts({ agentName: 'Chef (Chef)' }))
    expect(prompt).toContain('You are Chef (Chef)')
  ...
  it('includes plan state with proposal statuses', () => {
    ...
    const prompt = buildSystemPrompt('chef', session, opts({ agentName: 'Chef' }))

...
const dailyNoteRow: SearchResult = {
  ...
  fields: {
    ...
    source_backend: 'filesystem',
    source_path: '/openclaw/workspaces/scout/memory/daily/2026-04-17.md',
...
const auditRow: SearchResult = {
  ...
  fields: {
    ...
    agent: 'scout',
    title: 'task.complete',
    snippet: 'scout completed "embedding benchmark" task',
...
describe('memory plugin — cross-tier global search (C9-b)', () => {
  ...
  it('preserves row id, tier, agent, and meta through the route', async () => {
    ...
    expect(byId.get('session:abc123')?.fields.agent).toBe('scout')
    expect(byId.get('daily_note:scout:2026-04-17')?.fields.tier).toBe('daily_note')

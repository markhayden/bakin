    ...
    activated.seedResults([
      {
        ...
        table: 'bakin_audit',
        score: 1,
        fields: { event: 'task.created', agent: 'chef', content: 'task created' },
      },
    ])
...
describe('memory plugin — GET /audit route', () => {
  it('registers the route and returns parsed audit entries', async () => {
    ...
    writeFileSync(
      auditPath,
      [
        JSON.stringify({ ts: '2026-04-14T10:00:00Z', event: 'task.created', agent: 'chef', data: { id: 't1' } }),
        JSON.stringify({ ts: '2026-04-14T10:05:00Z', event: 'task.completed', agent: 'chef', data: { id: 't1' } }),
      ].join('\n'),
    )

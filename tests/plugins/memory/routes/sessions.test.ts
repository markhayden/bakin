...
describe('sessionsListRoute — handler', () => {
  ...
  it('lists sessions from the runtime store sorted by updatedAt desc', async () => {
    ...
    const { ctx } = makeCtx()
    const res = await sessionsListRoute.handler(req('/sessions', { agent: 'chef' }), ctx, {} as any)
...
describe('sessionDetailRoute — handler', () => {
  ...
  it('returns 400 when either param missing', async () => {
    const { ctx } = makeCtx()
    const res = await sessionDetailRoute.handler(req('/sessions/chef', { agent: 'chef' }), ctx, {} as any)
    expect(res.status).toBe(400)
  })
...
describe('turnsListRoute — handler', () => {
  ...
  it('returns 400 when agent or sessionId missing', async () => {
    const { ctx } = makeCtx()
    const res = await turnsListRoute.handler(req('/turns', { agent: 'chef' }), ctx, {} as any)
    expect(res.status).toBe(400)
  })
})

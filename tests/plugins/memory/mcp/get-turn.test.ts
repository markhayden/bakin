...
describe('memory_get_turn', () => {
  ...
  it('rejects missing turnId', async () => {
    const res = await createMemoryGetTurnTool(makeCtx()).handler({}, 'explorer')
    expect(res.ok).toBe(false)
  })

  it('rejects turnId without the turn: prefix', async () => {
    const res = await createMemoryGetTurnTool(makeCtx()).handler({ turnId: 'session:foo' }, 'explorer')
    expect(res.ok).toBe(false)
  ...
  it('returns the matched turn with parsed meta', async () => {
    ...
    const res = await tool.handler({ turnId: 'turn:def' }, 'explorer')
  ...
  it('returns not-found when the id is missing', async () => {
    const res = await createMemoryGetTurnTool(makeCtx([])).handler({ turnId: 'turn:missing' }, 'explorer')
    expect(res.ok).toBe(false)

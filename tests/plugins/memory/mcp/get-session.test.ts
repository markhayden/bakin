...
function sessionRow(sessionKey: string, agent = 'explorer'): SearchResult {
  return {
...
function turnRow(sessionKey: string, eventId: string, agent = 'explorer'): SearchResult {
  return {
    id: `turn:${eventId}`,
    table: 'bakin_memory',
    score: 0.5,
...
describe('memory_get_session — handler', () => {
  it('requires sessionKey', async () => {
    const { ctx } = makeCtx({})
    const res = await createMemoryGetSessionTool(ctx).handler({}, 'explorer')
  ...
  it('returns session + recent turns by matching meta.sessionKey', async () => {
    ...
    const res = await createMemoryGetSessionTool(ctx).handler({ sessionKey: 'sk1' }, 'explorer')
  ...
  it('returns not found when no session matches', async () => {
    ...
    const res = await createMemoryGetSessionTool(ctx).handler({ sessionKey: 'sk-missing' }, 'explorer')
    expect(res.ok).toBe(false)
    expect(res.error).toMatch(/not found/i)
  })

  it('passes agent filter down on both queries', async () => {
    const { ctx, calls } = makeCtx({ session: [sessionRow('sk1', 'chef')] })
    await createMemoryGetSessionTool(ctx).handler({ sessionKey: 'sk1', agent: 'chef' }, 'chef')
    expect(calls.every((c) => (c.filters as Record<string, string>).agent === 'chef')).toBe(true)
  })
})

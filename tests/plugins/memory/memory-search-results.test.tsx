...
function row(overrides: Partial<SearchResult> & { tier?: string; agent?: string; title?: string }): SearchResult {
  const { tier = 'session', agent = 'scout', title = 'A result', ...rest } = overrides
  return {
...
describe('MemorySearchResults', () => {
  ...
  it('renders one card per result with title, tier badge, and agent', () => {
    render(
      <MemorySearchResults
        results={[
          row({ id: 'session:a', tier: 'session', agent: 'scout', title: 'Session A' }),
          row({ id: 'daily_note:b', tier: 'daily_note', agent: 'basil', title: 'Daily B' }),
          row({ id: 'audit:c', tier: 'audit', agent: 'scout', title: 'Audit C' }),
        ]}
        loading={false}
        error={null}
        query="q"
    ...
    render(
      <MemorySearchResults
        results={[
          row({
            id: 'session:x',
            fields: { tier: 'session', agent: 'scout', title: 'X', snippet: 'unique-snippet-phrase' },
          }),
        ]}
        loading={false}
        error={null}

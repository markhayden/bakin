...
`)
  ...
  describe('parallel group', () => {
    ...
    it('does not leak a parallel sibling step to the wrong agent', () => {
      ...
      expect(getCurrentStep('task-par-wrong', 'nemo', testDir)).toBeNull()
    })
  })
  ...
  describe('agent-scoping', () => {
    ...
    it('authorizes progress logs only for the current owner', () => {
      ...
      expect(authorizeWorkflowToolUse('task-auth-log', 'basil', 'progress-log', testDir).allowed).toBe(true)
...
`)
      ...
      const early = authorizeWorkflowToolUse('task-auth-post', 'basil', 'channel-post', testDir)
      expect(early.allowed).toBe(false)
      expect(early.reason).toContain('only allowed from the active output step')

      completeStep('task-auth-post', 'write', { text: 'ready' }, 'basil', testDir)
      expect(authorizeWorkflowToolUse('task-auth-post', 'basil', 'channel-post', testDir).allowed).toBe(true)
    })

    it('denies task completion while a workflow is active', () => {
      createInstance('task-auth-complete', 'linear', testDir)

      const denied = authorizeWorkflowToolUse('task-auth-complete', 'basil', 'task-complete', testDir)
      expect(denied.allowed).toBe(false)

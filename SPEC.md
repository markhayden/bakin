...
The auto-memory at `~/.claude/projects/-Users-roscoe-go-src-github-com-madeinwyo-bakin/memory/` persists across Claude sessions. Stale `beacon` / `~/.beacon/` references will silently poison future sessions.
...
13. `~/.claude/projects/-Users-roscoe-go-src-github-com-madeinwyo-bakin/memory/` has no `beacon` references; `project_beacon_skill_rename.md` is absent.

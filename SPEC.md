...
everything Main Operator is doing, tracking, and thinking about. Accessible from
...
The core idea: Main Operator writes structured markdown files. A lightweight Node.js
...
└─────────────────────────┼────────────────────────────────────────┘
   ...
   │  Main Operator relays all agent      │
...
`~/go/src/github.com/markhayden/mission-control/`
...
~/.openclaw/workspaces/               ← Per-agent workspaces (OpenClaw managed, NOT in repo)
    ...
    │   ├── AGENTS.md, SOUL.md, IDENTITY.md, HEARTBEAT.md
    └── chef/                        ← 🥗 Chef — Food Content Creator
        ├── AGENTS.md, SOUL.md, IDENTITY.md, HEARTBEAT.md
```
...
- [ ] Write turmeric lentil soup recipe @chef — started 2026-03-18
- [ ] Coordinate daily content pipeline @main-operator — started 2026-03-18

## 📋 Todo
- [ ] Review Mission Control spec with Mark @main-operator
...
- [ ] Set up Discord channel structure @main-operator

## ✅ Done
- [x] Create Mission Control spec document @main-operator — 2026-03-18
- [x] Research Alex Finn Mission Control video @main-operator
...
- Main Operator updates this file; dashboard re-renders automatically via SSE
...
## Recurring
- Daily @ 09:00 MDT — Daily brief (Main Operator → #mc-daily-brief)
...
One markdown file per project. Main Operator creates these when new projects start
...
**Status:** 🟢 Active
**Goal:** Build a self-hosted real-time dashboard for Main Operator's activity
**Started:** 2026-03-18
...
A public-safe subset of Main Operator's memory. Decisions made, lessons learned,
...
Reference documents. SOPs, research notes, prompt templates, anything Main Operator
or Mark might need to reference. Main Operator can add docs here when creating
...
- **Model:** claude-sonnet-4-6
- **Workspace:** ~/.openclaw/workspaces/chef
- **Status:** 🟢 Active
...
The fun one. A 2D ASCII art map of a virtual office. Main Operator rebuilds this on
...
║   │  🎯       │ │  ⚙️       │ │  🖼️       │ │  🎬       │      ║
║   │ [Main Operator]  │ │ [Patch]   │ │ [Pixel]   │ │ [Rolo]    │      ║
...
|-------|--------|-------------|----------------|
| 🎯 Main Operator | 🟢 Working | Coordinating daily content pipeline | 14:45 MDT |
...
| 🥗 Chef | 🟢 Working | Writing turmeric lentil soup recipe | 14:43 MDT |

## Status History
- 14:45 — Main Operator: Working — Coordinating daily content pipeline
- 14:43 — Chef: Working — Writing turmeric lentil soup recipe
- 14:44 — Patch: Working — Building nanobanan integration
- 14:40 — Pixel: Idle — Waiting for image brief from Chef
- 14:40 — Rolo: Idle — Waiting for video brief from Chef
```
...
- Main Operator emoji moves position based on activity type (at desk = working,
...
const server = http.createServer((req, res) => {
  ...
  if (req.url.startsWith('/api/agents/')) {
    // e.g., execSync('openclaw agent --agent main-operator --message "..." --deliver')
  }
...
```bash
cd ~/go/src/github.com/markhayden/mission-control
node server.js
```
...
<dict>
  ...
  <array>
    ...
    <string>/Users/dev/go/src/github.com/markhayden/mission-control/server.js</string>
  </array>
  <key>RunAtLoad</key>
  <true/>
  <key>KeepAlive</key>
...
Mission Control manages multiple OpenClaw agents. **Main Operator is the orchestrator** —
...
// ~/.openclaw/openclaw.json — agents section
{
  ...
  "tools": {
    "agentToAgent": {
      "enabled": true,
      "allow": ["main-operator", "patch", "pixel", "rolo", "chef"]
    }
  }
}
```
...
|-------|------|-------|-------|
| 🎯 Main Operator | Orchestrator | sonnet | "Alright crew, here's today's mission..." |
...
| 🥗 Chef | Food Content Creator | sonnet | "Today we're making a 20-minute turmeric lentil soup..." |

### Agent Responsibilities

**🎯 Main Operator — The Orchestrator**
- Receives high-level goals, breaks them into tasks
- Delegates to Patch, Pixel, Rolo, Chef
...
- Receives video briefs and scripts from content agents (primarily Chef)
...
**🥗 Chef — The Nutritionist & Food Content Creator**
...
- Hands completed packages to Main Operator for scheduling and publishing
- Content pillars: Recipes, Nutrition education, Meal planning, Ingredient spotlights, Health infographics

### Content Pipeline Flow

```
Chef (plans content, writes copy)
  ├──▶ Pixel (image brief) ──▶ nanobanan ──▶ assets back to Chef
  ├──▶ Rolo (video brief) ──▶ video LLM ──▶ assets back to Chef
  │         └──▶ Pixel (still frames needed for video)
  │
  Chef (assembles final post package)
  └──▶ Main Operator (scheduling + publishing across platforms)

Patch supports the whole pipeline:
  - builds integrations, fixes bugs, extends tooling
  - called by Main Operator when something breaks or needs automation
```
...
1. **Sub-agent spawning** — Main Operator delegates tasks:
   ```
   ...
   /subagents spawn chef "Plan next week's content calendar"
   ```
...
5. **Dashboard writes** (from POST endpoints) also go through Main Operator.
...
can also trigger Main Operator directly via the OpenClaw gateway API:
```bash
openclaw agent --agent main-operator --message "Mark assigned 'Build MC server v1' to patch" --deliver
```
...
### Implementation
- Task endpoints write changes to `content/inbox/` for Main Operator to process
...
**Who writes:** Main Operator appends to this file whenever it updates shared state.
The MC server also appends directly for dashboard-initiated actions.

**Event format:**
```jsonl
{"ts":"2026-03-18T14:30:00-06:00","event":"task.created","agent":"main-operator","data":{"title":"Build MC server v1","assignee":"patch","column":"todo"}}
{"ts":"2026-03-18T14:31:00-06:00","event":"task.moved","agent":"main-operator","data":{"title":"Build MC server v1","from":"todo","to":"in_progress"}}
...
{"ts":"2026-03-18T14:35:00-06:00","event":"heartbeat","agent":"chef","data":{"status":"working","task":"Writing turmeric lentil soup recipe"}}
{"ts":"2026-03-18T14:40:00-06:00","event":"task.completed","agent":"patch","data":{"title":"Build nanobanan integration","duration_min":120}}
{"ts":"2026-03-18T14:41:00-06:00","event":"agent.health","agent":"main-operator","data":{"target":"rolo","status":"stale","missed_heartbeats":3}}
{"ts":"2026-03-18T14:42:00-06:00","event":"agent.restart","agent":"main-operator","data":{"target":"rolo","reason":"missed 3 heartbeats"}}
{"ts":"2026-03-18T15:00:00-06:00","event":"asset.created","agent":"pixel","data":{"file":"2026-03-18-lentil-soup-hero.jpg","brief_from":"chef","type":"image"}}
{"ts":"2026-03-18T15:10:00-06:00","event":"content.published","agent":"main-operator","data":{"platform":"discord","channel":"#recipes","assets":["2026-03-18-lentil-soup-hero.jpg"]}}
```

**Event types:**
| Event | Description | Writer |
|-------|-------------|--------|
| `task.created` | New task added to board | Main Operator / MC server |
| `task.moved` | Task moved between columns | Main Operator / MC server |
| `task.assigned` | Task assigned to agent | Main Operator / MC server |
| `task.completed` | Task marked done | Main Operator |
| `task.blocked` | Task blocked | Main Operator |
| `heartbeat` | Agent heartbeat received | Main Operator |
| `agent.health` | Health status change (stale, recovered) | Main Operator |
| `agent.start` | Agent started | MC server |
| `agent.stop` | Agent stopped | MC server |
| `agent.restart` | Agent restarted | Main Operator / MC server |
| `asset.created` | Image/video generated | Pixel / Rolo (via Main Operator) |
| `content.published` | Content posted to platform | Main Operator |
| `decision` | Significant decision logged | Any agent |
| `inbox.processed` | Dashboard inbox item handled | Main Operator |
...
**Rotation:** When `audit.jsonl` exceeds 10MB, Main Operator rotates it to
...
**Workflow:**
1. Chef sends image brief to Pixel via agent-to-agent message
2. Pixel generates image via nanobanan, saves to `assets/`
3. Pixel logs `asset.created` event (via Main Operator)
4. Pixel notifies Chef that asset is ready
5. Chef assembles final post package referencing the asset path
...
| Calendar event | CALENDAR.md | Main Operator only |
| New project | projects/<slug>.md | Main Operator only |
| Any state change | audit.jsonl | Main Operator + MC server (append-only) |
...
Discord is bidirectional. Main Operator is the single bot — no need for per-agent bots.
...
```
@main-operator assign "Generate lentil soup hero image" to pixel
@main-operator status
...
🔵 STARTED: @chef started "Write turmeric lentil soup recipe"
```
...
🤖 Agent Status
  🎯 Main Operator: 🟢 | ⚙️ Patch: 🟢 | 🖼️ Pixel: 🟡 | 🎬 Rolo: 🟡 | 🥗 Chef: 🟢

📋 Active Tasks (3)
  • Coordinate daily content pipeline @main-operator
  • Build nanobanan integration @patch
  • Write turmeric lentil soup recipe @chef

📅 Today
  • 09:00 — This brief (done ✅)
  • 10:00 — Chef posts recipe content
...
**Agent setup:**
- [ ] Add agents to `openclaw.json` (patch, pixel, rolo, chef)
- [ ] Create workspaces: `~/.openclaw/workspaces/{patch,pixel,rolo,chef}/`
...
- [ ] Update Main Operator's AGENTS.md with orchestrator responsibilities
- [ ] Update Main Operator's AGENTS.md with content pipeline flow
...
- [ ] Wire Main Operator to post task state changes to #mc-tasks
- [ ] Wire Main Operator to post health alerts to #mc-alerts
...
### Repo
When ready to build: `~/go/src/github.com/markhayden/antfly-integration/`
...
| Shared file write strategy | Main Operator-only writes + inbox pattern | ✅ Decided |

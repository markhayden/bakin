...
Path aliases: `@/*` maps to `./src/*` for server-side code, and core plugins use `@bakin/{plugin}/*` only inside Bakin app code/tests. External plugin authors never use `@/*` or `@bakin/{plugin}/*` — the canonical entry point is `@bakin/sdk/*`.

...
- **CLI:** Two files cooperate. `src/core/cli.ts` is the binary-facing dispatcher (`start`/`stop`/`status`/`version`/`update`/`plugins {…}`/`dev`); unknown commands delegate to `cli/bakin.ts`, the HTTP-client CLI that owns `doctor`, `dispatch`, `tasks`, `agents`, `packages`, `schedule`, `messaging`, `search`, `trash`, `settings`, `logs`, `paths`, `reindex`, `restart`, `docs`, `onboard`, `init`, `mkdir`, `check`, `install`. Use `bakin --help` for the full surface.

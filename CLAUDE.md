...
- **Onboarding** — `src/core/onboarding/` — 14 components (the `COMPONENT_ORDER` array; the `budget` component prompts for spending caps — skippable, never a silent default) with shared `check()` + `install()` contract. `bakin install search` downloads the pinned engine AND provisions the OS service (LaunchAgent/systemd user unit; upgrades are stop→swap→restart). CLI: `bakin onboard --yes` for non-interactive setup; `bakin mkdir`, `bakin install {search,search-models,mcporter,plugin-assets,agent-sync}`, `bakin check {runtime,search,search-models,llm,channels,plugin-assets,agent-sync,all}`, `bakin settings init`. Doctor gates on `~/.bakin/.onboarded` marker.

...
- **MCP Tool Registration** — Fully dynamic. The exec-tool registry is `src/core/exec-tools/registry.ts` (runtime code; moved out of `scripts/lib/` in WS2). Plugins register exec tools via `ctx.registerExecTool()` during activation; the `scripts/lib/*` tool peers self-register via `addExecTool()` on import; `mcp-server.ts` calls `getAllExecTools()` at startup.

...
- **Chat** — The `chat` core plugin is the in-app conversational surface: streamed multi-chat with any agent, runtime-agnostic via `runtime.messaging.stream` with threadId `chat:<chatId>`. Transcripts are Bakin-owned UI data (`~/.bakin/chat/`: zod `index.json` + per-chat JSONL); turns stream as `chat.chunk`/`chat.done`/`chat.error` plugin-events over the shared SSE bus (`usePluginEvent` client-side); one in-flight turn per chat (409). Deep reference: `.claude/knowledge/chat-plugin.md`.
...
- **Onboarding** — `src/core/onboarding/` — 13 components (the `COMPONENT_ORDER` array; the `budget` component prompts for spending caps — skippable, never a silent default) with shared `check()` + `install()` contract. `bakin install search` downloads the pinned engine AND provisions the OS service (LaunchAgent/systemd user unit; upgrades are stop→swap→restart). CLI: `bakin onboard --yes` for non-interactive setup; `bakin mkdir`, `bakin install {search,search-models,plugin-assets,agent-sync}`, `bakin check {runtime,search,search-models,llm,channels,plugin-assets,agent-sync,all}`, `bakin settings init`. Doctor gates on `~/.bakin/.onboarded` marker.
...
- **Agent Health Diagnostics** — The supervision layer (#385). ONE engine per concern, every surface a thin client: `src/core/agent-burn.ts` (warn-only effort/spike/**unattributed** heuristics — usage.db-observed minus run_costs-attributed = activity outside Bakin-managed tasks; NULL-honest, day-aligned, `settings.burn` tunables), `scanAgentSync` (live per-agent scan endpoint + cached doctor fleet state), `context-report.ts`, and `plugins/team/lib/timeline.ts` (run spine + audit interleave). Surfaces: health dashboard (live-now / attention chips / effort / stacked daily chart), Team Diagnostics tab + Overview chips + `drifted` badge, `bakin agents doctor <id>`. Per-agent doctor findings attach `HealthCheckResult.data.agents` — UIs never parse message text. Chart kit (`StackedColumnChart`/`Sparkline`/`ChartExplainer`) is hand-rolled in the SDK — no chart library; the palette slot order is the CVD-safety mechanism, never reorder. Deep reference: `.claude/knowledge/agent-health-diagnostics.md`.
